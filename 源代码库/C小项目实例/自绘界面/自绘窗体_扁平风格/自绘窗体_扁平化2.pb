...
Procedure Screen_Redraw(IsRecreate)
   With _Screen
      If IsRecreate = #True Or IsImage(\TitleImageID) = 0
         ...
         Screen_btnMinimize(\btnMinimize, \WindowW-5-36*2, 0, 36, 32, \TitleImageID)
      EndIf  
      ...
      ElseIf Macro_Gadget_InRect(\btnMinimize)
         If \pHoldDown = \btnMinimize
            *pEventGadget = \btnMinimize
            SetWindowState(#winScreen, #PB_Window_Minimize)
         EndIf          
      EndIf 
      ;整理响应事件
      If \pHoldDown <> *pEventGadget
...
With _Screen
   ...
   \WindowW = 550
   \WindowH = 350
   \hWindow = OpenWindow(#winScreen, 0, 0, \WindowW, \WindowH, "自绘窗体_扁平化2", WindowFlags)
