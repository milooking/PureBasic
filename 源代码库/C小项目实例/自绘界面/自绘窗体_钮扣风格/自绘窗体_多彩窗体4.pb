...
Procedure Screen_Redraw(IsRecreate)
   With _Screen
      ...
      If StartDrawing(ImageOutput(ImageID))
         ...
         DrawingMode(#PB_2DDrawing_Outlined)
         Box(000, 000, \WindowW+0, \WindowH+0, #Screen_LineColor)
         Box(003, 003, \WindowW-6, \WindowH-6, #Screen_SideColor)         
         ...
         For i = 15 To 03 Step -4
            For j = i To 03 Step -4
               Box(\WindowW-24+i, \WindowH-j-6, 2, 2, $FF000000|#Screen_ForeColor)
            Next 
         Next 
...
Procedure Screen_Hook_MOUSEMOVE(*pMouse.POINTS)
   With _Screen
      ...
      ElseIf *pMouse\x >= \WindowW-16 And *pMouse\y >= \WindowH-16
          SetCursor_(\hSizing)      
      ElseIf *pMouse\x <= 3 Or *pMouse\x >= \WindowW-3
          SetCursor_(\hLeftRight)
      ElseIf *pMouse\y <= 3 Or *pMouse\y >= \WindowH -3
          SetCursor_(\hUpDown)          
      EndIf 
      ;整理响应事件
...
Procedure Screen_Hook_LBUTTONDOWN(*pMouse.POINTS)
   With _Screen
      ...
      ElseIf *pMouse\x >= \WindowW-16 And *pMouse\y >= \WindowH-16
          SetCursor_(\hSizing)   
          SetTimer_ (\hWindow, #WM_SIZE, 5, #Null) 
...
Procedure Screen_Hook(hWindow, uMsg, wParam, lParam) 
   With _Screen
      ...
      Select uMsg 
         ...
         Case #WM_TIMER         : Screen_Hook_TIMER      (wParam)
...
With _Screen
   \hSizing    = LoadCursor_(0,#IDC_SIZENWSE)
   \hLeftRight = LoadCursor_(0,#IDC_SIZEWE)
   \hUpDown    = LoadCursor_(0,#IDC_SIZENS)
   WindowFlags = #PB_Window_BorderLess|#PB_Window_ScreenCentered
   \WindowW = 400
   \WindowH = 250
   \hWindow = OpenWindow(#winScreen, 0, 0, 400, 250, "自绘窗体_多彩窗体4", WindowFlags)
   ...
   Repeat
      ...
      EndSelect
      KillTimer_(_Screen\hWindow, #WM_SIZE) 
      Delay(1)   
