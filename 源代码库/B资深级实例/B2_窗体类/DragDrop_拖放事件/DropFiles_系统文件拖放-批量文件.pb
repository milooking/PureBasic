...
Procedure Funciton_EnumFiles(FilePath$)
   ...
   If DirID 
      While NextDirectoryEntry(DirID)  
         ...
         If DirectoryEntryType(DirID) = #PB_DirectoryEntry_File   ;获取文件名
            Count + 1      
            AddGadgetItem(#lvwScreen, -1, FilePath$+FileName$)
         ElseIf DirectoryEntryName(DirID) = "."                   ;排除DOS兼容格式
         ElseIf DirectoryEntryName(DirID) = ".."                  ;排除DOS兼容格式  
         Else                                                     ;文件夹
            Count + Funciton_EnumFiles(FilePath$+FileName$+"\")  
         EndIf  
      Wend  
...
hWindow = OpenWindow(#winScreen, 0, 0,600, 350, "系统文件拖放-批量文件", WindowFlags)
