   ...
   With _Screen
      If StartDrawing(CanvasOutput(#cvsScreen))
         ...
         *pColor.__ScreenColorInfo = @\DimColor[1]
         ...
         FrontColor(*pColor\BackColor)
         ;上下性线渐变
         LinearGradient(0, 0, 0, #CaptionH) 
...
;挂钩事件
Procedure Screen_HookGadget(hGadget, uMsg, wParam, lParam) 
   With _Screen
      If \hCanvas <> hGadget
      ...
      EndSelect 
      If Result = 0 
         Result = CallWindowProc_(\hGadgetHook, hGadget, uMsg, wParam, lParam) 
      EndIf
   EndWith
   ProcedureReturn Result
EndProcedure
   ...
   With *pGadget
      ...
      *pColor.__ScreenColorInfo = @_Screen\DimColor[1]
      ...
      If StartDrawing(ImageOutput(\MouseTopID))
         DrawingMode(#PB_2DDrawing_AlphaBlend)
         RoundBox(0, -5, \W, \H+5, 3, 3, #SysButton_CloseColor)     ;背景渲染
      ...
      If StartDrawing(ImageOutput(\HoldDownID))
         DrawingMode(#PB_2DDrawing_AlphaBlend)
         RoundBox(0, -5, \W, \H+5, 3, 3, CloseColor)     ;背景渲染
         DrawAlphaImage(ImageID(\NormalcyID), 0, 0)
         StopDrawing()
      EndIf
   EndWith
...
Procedure Create_btnMinimize(*pGadget.__GadgetInfo)
   With *pGadget
      ...
      *pColor.__ScreenColorInfo = @_Screen\DimColor[1]
      ...
      If StartDrawing(ImageOutput(\MouseTopID))
         DrawingMode(#PB_2DDrawing_AlphaBlend)
         RoundBox(0, -5, \W, \H+5, 3, 3, *pColor\ForeColor)     ;背景渲染
         DrawAlphaImage(ImageID(\NormalcyID), 0, 0)
         StopDrawing()
      EndIf
      ;绘制左键按下时的控件图像
      ...
      If StartDrawing(ImageOutput(\HoldDownID))
         DrawingMode(#PB_2DDrawing_AlphaBlend)
         RoundBox(0, -5, \W, \H+5, 3, 3, ForeColor)     ;背景渲染
         DrawAlphaImage(ImageID(\NormalcyID), 0, 0)
         StopDrawing()
      EndIf
   EndWith
...
With _Screen
   ...
   SetWindowColor(#winScreen, _Screen\DimColor[0]\BackColor & $FFFFFF)
   Create_btnCloseBox(\btnCloseBox)    ;创建关闭窗体小按键
   Create_btnMinimize(\btnMinimize)    ;最小化窗体小按键
   \hCanvas = CanvasGadget(#cvsScreen, 0, 0, \WindowW, \WindowH, #PB_Canvas_Container)
      ;这里可以添加所需要的控件...
   CloseGadgetList()
   Redraw_Screen()
   \hGadgetHook = SetWindowLongPtr_(\hCanvas, #GWL_WNDPROC, @Screen_HookGadget())
      ...
      Select EventNum
         ...
         Case #Null
            If \pMouseTop Or \pHoldDown
               ...
               If WindowFromPoint_(Mouse) <> \hCanvas
