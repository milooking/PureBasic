...
Procedure Redraw_Graphics(IsShow=#False)
   If StartVectorDrawing(CanvasVectorOutput(#cvsScreen))
      ...
      ;绘制辅助点
      For k = 0 To 5
         If _DimPoint(k) = _Graphics\pPoint : Continue : EndIf 
         AddPathBox(_DimPoint(k)\x-5, _DimPoint(k)\y-5, 10, 10)
      Next
...
Procedure EventGadget_cvsScreen()
   ...
   Select EventType()
      Case #PB_EventType_LeftButtonDown
         ...
         For k = 0 To 5
            If x >= _DimPoint(k)\x-5 And x<=_DimPoint(k)\x-5+10 And y>=_DimPoint(k)\y-5 And y<=_DimPoint(k)\y-5+10
...
If OpenWindow(#winScreen, 0, 0, 400, 300, "矢量绘图_双曲线多点调整1", #PB_Window_SystemMenu | #PB_Window_ScreenCentered)
